...
export const MultiComboBox: FC<Props> = ({
  ...
  const focusableDropdownCount = (dropdownContentFlags.addable ? 1 : 0) + filteredItems.length

  useEffect(() => {
    // correct the focus index in dropdown to fit actual row count
    setDropdownFocusIndex(Math.max(Math.min(dropdownFocusIndex, focusableDropdownCount - 1), 0))
  }, [dropdownFocusIndex, focusableDropdownCount])

  const focusDropdownItem = useCallback(
    (index: number) => {
      setDropdownFocusIndex((index + focusableDropdownCount) % focusableDropdownCount)
    },
    [focusableDropdownCount],
  )
              ...
              <Input
                ...
                onKeyDown={(e) => {
                  ...
                  switch (e.key) {
                    ...
                    case 'Down':
                      e.preventDefault()
                      focusDropdownItem(dropdownFocusIndex + 1)
                      break
                    case 'ArrowUp':
                    case 'Up':
                      e.preventDefault()
                    ...
                    case 'Enter': {
                      ...
                      if (addable && dropdownFocusIndex === 0) {
                        onAdd && onAdd(inputValue)
                        return
                      }
                      const itemIndex = dropdownFocusIndex - (addable ? 1 : 0)
          ...
          <Dropdown themes={theme} ref={dropdownRef} width={dropdownStyle.width}>
            {dropdownContentFlags.addable && (
              <AddButton
                ...
                className={dropdownFocusIndex === 0 ? 'active' : undefined}
              >
                <AddIcon
                  name="fa-plus-circle"
                  size={14}
            ...
            {dropdownContentFlags.viewList &&
              filteredItems.map(({ label, value, disabled: itemDisabled = false }, i) => (
                <SelectButton
                  ...
                  className={
                    dropdownFocusIndex === i + (dropdownContentFlags.addable ? 1 : 0)
                      ? 'active'
                      : undefined
                  }
                >
